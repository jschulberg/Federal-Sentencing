...
https://github.com/khwilson/SentencingCommissionDatasets
...
def main():
    ...
    for filename in os.listdir(file_path):
        if '.zip' in filename and filename.split('.')[0] + '.csv' not in os.listdir('Data/Processed'):
            start_time = time.time()
            
            try:
                print(f'Converting {filename}...')
                convert_file(file_path + filename)
                
                # After we're done writing everything to CSV, if they only exist in 
                # our main data folder, move them over to our 'Data/Processed' folder
                print(f"Moving {filename} to 'Data/Processed folder")
                os.rename(file_path + filename.split('.')[0] + '.csv', 'Data/Processed/' + filename.split('.')[0] + '.csv')
